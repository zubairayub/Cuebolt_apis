...
class TradesController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Step 1: Validation with X Input Errors
        ...
        try {
            ...
            if ($package->user_id !== auth()->id()) {
                ...
                return redirect()->back()->with('error', 'Unauthorized to create trade for this package');
            }

            // Process take profit values
            $takeProfit1 = $validated['take_profit'][0] ?? null;
            $takeProfit2 = $validated['take_profit'][1] ?? null;
            ...
            if ($request->hasFile('images')) {
                ...
                foreach ($request->file('images') as $image) {
                    $filePath = $image->store($basePath, 'public');
                    $trade->images()->create(['image_path' => $filePath, 'image_name' => $image->getClientOriginalName()]);
                }
            }
            ...
            $usersWithTokens = User::whereIn('id', $userIds)->whereNotNull('fcm_token')->get(['id', 'fcm_token']);
            ...
            if ($tokens->isNotEmpty()) {
                ...
                send_push_notification(
                    $tokens->toArray(),
                    "{$ownerUsername} shared a new signal",
                    "📈 Market Pair: {$marketPairName}\n🔹 Trade Type: {$tradeTypeName}\n💰 Entry Price: {$validated['entry_price']}\n📉 Stop Loss: {$validated['stop_loss']}",
                    [
                        'trade_id' => $trade->id,
                        'package_id' => $validated['package_id'],
                        'type' => 'trade_notification',
            ...
            Log::error('Trade creation failed:', ['message' => $e->getMessage()]);
            return redirect()->back()->with('error', 'Trade creation failed: ' . $e->getMessage());
        }
    }
